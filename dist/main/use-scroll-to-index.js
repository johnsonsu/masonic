...
var _event = /*#__PURE__*/ _interopRequireDefault(
  /*#__PURE__*/ require("@react-hook/event")
);
...
function useScrollToIndex(positioner, options) {
    ...
    if (action.type === "scrollToIndex") {
      ...
      return {
        position: latestOptions.current.positioner.get(
          (_action$value = action.value) !== null && _action$value !== void 0
            ? _action$value
            : -1
        ),
        index: action.value,
  ...
  const throttledDispatch = (0, _throttle.useThrottleCallback)(dispatch, 15); // If we find the position along the way we can immediately take off
  // to the correct spot.
